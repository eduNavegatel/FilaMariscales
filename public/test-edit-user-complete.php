<?php
// Test completo de edición de usuarios
...
echo "<title>Test Completo Edición Usuarios - Filá Mariscales</title>";
echo "<link href='https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/css/bootstrap.min.css' rel='stylesheet'>";
echo "</head>";
echo "<body>";
echo "<div class='container mt-4'>";
echo "<h1>🧪 Test Completo de Edición de Usuarios</h1>";
...
try {
    ...
    if ($user) {
        ...
        echo "<div class='card'>";
        echo "<div class='card-body'>";
        echo "<form id='testEditForm' method='POST' action='/prueba-php/public/admin/editarUsuario/{$userId}'>";
        ...
        echo "<button type='button' class='btn btn-secondary' onclick='testFormData()'>Probar Datos del Formulario</button>";
        echo "<button type='button' class='btn btn-info' onclick='testUpdateUser()'>Test Actualización</button>";
        echo "<button type='button' class='btn btn-warning' onclick='testAdminController()'>Test AdminController</h3>";
        echo "</div>";
        echo "</form>";
        echo "</div>";
        echo "</div>";
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            try {
                ...
                error_log("=== INICIO DEL TEST COMPLETO DE EDICIÓN ===");
                error_log("Request method: " . $_SERVER['REQUEST_METHOD']);
                error_log("POST data: " . print_r($_POST, true));
                error_log("URL path: " . $_SERVER['REQUEST_URI']);
                ...
                if (empty($userData['errors'])) {
                    ...
                    if ($result) {
                        ...
                        echo "<tr><td>Apellidos</td><td>{$user->apellidos}</td><td>{$updatedUser->updatedUser->apellidos}</td></tr>";
                ...
                error_log("=== FIN DEL TEST COMPLETO DE EDICIÓN ===");
...
echo "    if (confirm('¿Estás seguro de que quieres actualizar el usuario directamente?')) {";
echo "        document.getElementById('testEditForm').submit();";
echo "    }";
echo "}";
echo "";
echo "function testAdminController() {";
echo "    const formData = new FormData(document.getElementById('testEditForm'));";
echo "    const data = {};";
echo "    for (let [key, value] of formData.entries()) {";
echo "        data[key] = value;";
echo "    }";
echo "    console.log('Datos que se enviarían al AdminController:', data);";
